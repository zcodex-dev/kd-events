...
export async function POST(request: Request) {
  try {
    ...
    await updateAppConfig({
      ...currentConfig,
      enrollmentBgUrl: imageUrl,
    });

    return NextResponse.json<ApiResponse<{ url: string }>>(
      { success: true, data: { url: imageUrl }, message: 'Enrollment background updated successfully' },
      { status: 200 }
    );
  } catch (error) {
    console.error('Enrollment BG upload error:', error);
    return NextResponse.json<ApiResponse>(
