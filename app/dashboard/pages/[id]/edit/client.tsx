...
export function EditPageClient({ initialData }: { initialData: WebPage }) {
  const { openSidebar, session } = useDashboard();

  if (session && session.role !== 'admin') {
    return (
      <>
        <Header title="Edit Web Page" onMenuClick={openSidebar} />
        <div className="max-w-md mx-auto mt-20 p-6 bg-white border border-neutral-200 rounded-2xl shadow-sm text-center">
          ...
          <p className="text-xs text-neutral-500 mt-2">
            Only administrators can edit web pages.
          </p>
        </div>
      </>
    );
  ...
  return (
    <>
      <Header
        title="Edit Web Page"
        description="Update your Terms & Conditions or custom page"
      ...
      >
        <PageForm initialData={initialData} isEdit />
      </motion.div>
    </>
  );
}
