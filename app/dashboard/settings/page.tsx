...
export default function SettingsPage() {
  ...
  const [enrollmentBgUrl, setEnrollmentBgUrl] = useState<string | null>(null);
    ...
    fetch('/api/config')
      ...
      .then((resData) => {
        if (resData.success && resData.data) {
          ...
          if (resData.data.enrollmentBgUrl) setEnrollmentBgUrl(resData.data.enrollmentBgUrl);
        } else {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Enrollment Background Upload Section */}
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-xs overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-neutral-200 dark:border-neutral-800 flex items-center gap-2">
            <ImageIcon className="w-4 h-4 text-blue-600 dark:text-blue-500" />
            <div>
              <h2 className="text-sm font-semibold text-neutral-900 dark:text-white">
                Member Enrollment Background
              </h2>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                Upload a custom high-resolution background for the member enrollment page.
              </p>
            </div>
          </div>
